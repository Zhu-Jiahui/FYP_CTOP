...
void testTotalCustomers(customer CUSTOMERS[], list<int> vehicles[], list<int>* unassignCustomerList) {
    ...
    for (int i = 0; i < MAXVEHICLES; i++) {
        iter = vehicles[i].begin();
        cout << "vehicle " << i << " has capacity of " << getVehicleCapacity(CUSTOMERS, vehicles[i]) << " . and duration of " << getVehicleTimeTaken(CUSTOMERS, vehicles[i]) << " and contents customers: ";
    ...
    cout << "unassign vehicle contents: ";
...
void Reversion(customer CUSTOMERS[], list<int>* unassignCustomersList, list<int> vehicles[]){
    ...
    //cout << "current arrange size is " << currentArrangement.size() << '\n';
    ...
    vehicleNo = 0;
    while (currentArrangement.empty() == false && vehicleNo < MAXVEHICLES) {
         //cout << "Processing Vehicle no. " << vehicleNo << '\n';
...
void localInsertUnassign(customer CUSTOMERS[], list<int> vehicles[], list<int>* unassignCustomerList) {
    ...
    //first check if there is any empty vehicle caused by previous local search.
    ...
    while (N < N_nonImproving) {
        for (I = 0; I < Imax; I++){
            ...
            if (offsetProfit >= 0) { // replace with current vehicle
                ...
                //cout << "r is " << r << '\n';
                equationValue = exp((nextProfit - currentProfit)/T);
